...
def create_html_heatmap(tokens, shap_scores, title):
    ...
    for token, score in zip(tokens, shap_scores):
        ...
        if score > 0:
            color = f"rgba(255, 99, 71, {alpha})"
        else:
            color = f"rgba(100, 149, 237, {alpha})"
...
def find_extreme_false_negatives(model_path, df, device, top_n=3):

    print(f"\nFind the {top_n} most extreme false negatives using RoBERTa...")
    ...
    extreme_fns = ai_df.sort_values(by='ai_prob', ascending=True).head(top_n)
...
def analyze_model_shap(model_path, text, device):
    ...
    scores_ai = shap_values.values[0][:, 1] 
    
    return tokens, scores_ai
...
def main():
    ...
    if not os.path.exists(news_path):
        print(f"ERROR: Record not found under {news_path}")
        return
        
    df = pd.read_csv(news_path)
    
    extreme_cases = find_extreme_false_negatives(roberta_path, df, device, top_n=3)
    ...
    <head>
        <meta charset="utf-8">
        <title>Top 3 Extreme False Negatives: DistilBERT vs RoBERTa</title>
        <style>
    ...
    <body>
        <h1 style="font-family: 'Times New Roman', serif; text-align: center;">SHAP Architectural Comparison: Extreme False Negatives</h1>
    ...
    for i, (index, row) in enumerate(extreme_cases.iterrows()):
        text = row['text']
        ai_prob = row['ai_prob']
        print(f"\n[{i+1}/3] Analysiere Text (AI Probability: {ai_prob:.4f})...")
        
        print("  -> Berechne DistilBERT SHAP...")
        tokens_db, scores_db = analyze_model_shap(distilbert_path, text, device)
        
        print("  -> Berechne RoBERTa SHAP...")
        tokens_rob, scores_rob = analyze_model_shap(roberta_path, text, device)
        
        html_content += f"<div class='case-container'>"
        html_content += f"<h2 style='font-family: \"Times New Roman\", serif; color: #800000;'>Edge-Case #{i+1} (Model Confidence that this is AI: {ai_prob*100:.2f}%)</h2>"
    ...
    output_path = os.path.join(output_dir, "shap_top3_false_negatives.html")
